    ...
    if (cxc == null || cxc.length < 1) {
      ...
      while (elements.hasMoreElements()) {
        ...
        for (int i = 0; i < cxc.length; i++) {
          if (cxc[i].equals(fm.getFeatureName()) && fm.getInterfaceName() != null) {

              if (fm.getInterfaceName().startsWith("INTF")) {
                  // make sure not to add interfaces multiple times.
                  if (!ifaces.contains(fm.getInterfaceName())) {
                    ifaces.add(fm.getInterfaceName());
                  }
                } else {
                  log.finest("Ignoring " + fm.getInterfaceName() + " because it is not an interface.");
                }              
          }
        }
      }
