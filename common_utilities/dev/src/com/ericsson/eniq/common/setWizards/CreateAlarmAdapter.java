...
public class CreateAlarmAdapter {
  ...
  private Logger log = Logger.getLogger(CreateAlarmAdapter.class.getName());
  ...
  private void createAdapterSet() {
    this.log.finest("Starting to create Alarm Set Adapter_" + this.interfaceName + "_alarm.");
    try {
      ...
      this.log.finest("Alarm Set Adapter_" + this.interfaceName + "_alarm created succesfully.");

    } catch (Exception e) {
      this.log.log(Level.SEVERE, "Alarm Set Adapter_" + this.interfaceName + "_alarm creation failed", e);
    }
  }
