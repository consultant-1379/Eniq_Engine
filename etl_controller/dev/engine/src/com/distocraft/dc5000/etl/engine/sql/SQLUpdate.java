...
public class SQLUpdate extends SQLOperation {
    ...
    *  @since  JDK1.1
    */
    public SQLUpdate(       Meta_versions         version,
	    ...
	    (this.target.getTableName().equals(this.source.getTableName())==true) ||
            ...
            throw new EngineMetaDataException(  EngineConstants.NO_PRIMARY_KEY_DEFINED,
                                                new String[]{this.source.getTableName(), this.target.getTableName()},
                                                null,
                                                this,
    ...
    */
    protected void executeThroughJava()throws EngineException {
            try {
                ...
                RockResultSet results = this.source.getConnection().setSelectSQL(this.updateSelectClause);
                Vector objVec = this.source.getSelectObjVec(results,true);
                String preparedSqlStr = this.getPreparedUpdateClause(false);
                String preparedWhereStr = this.target.getPkPreparedWhereClause(false);
            ...
            throw new EngineException(  EngineConstants.CANNOT_EXECUTE,
                                        new String[]{this.getUpdateSelectClause()},
                                        e,
                                        this,
                                        this.getClass().getName(),
                                        EngineConstants.ERR_TYPE_EXECUTION);
    ...
    *
    */
    public String getPreparedUpdateClause(boolean useTargetAliasName) {
        String updateStr = "";
        ...
        if (this.target.getJoinedColumns() != null) {
            ...
            for (int i=0; i < this.target.getJoinedColumns().size(); i++) {
                Meta_joints joint = this.target.getJoinedColumns().getElementAt(i);
                
                    Meta_columns targetColumn = (Meta_columns)this.target.getColumns().elementAt(i);
	...
	private void setSybaseUpdateClause() {
            ...
            int counter = 0;
            for (int i=0; i<this.target.getJoinedColumns().size(); i++) {
                String targetColumn = this.target.getColumnName(i,false,true);
                String sourceColumn = this.source.getSourceColumn(i,false,true,false);
	    ...
	    String pkWhereClause = this.target.getPkWhereClause(this.source).trim();
	    String whereClause = this.source.getWhereClause().trim();

	    if (pkWhereClause.length() > 0 ||
               (this.target.getTableName().equals(this.source.getTableName())==false)) {
