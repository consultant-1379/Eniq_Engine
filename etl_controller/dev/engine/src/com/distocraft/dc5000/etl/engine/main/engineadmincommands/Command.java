...
/**
 * Base command for all CLI engine -e commands
 * For each possible command that the user can enter with engine -e, there should be a corresponding
 * subclass of Command (see EngineAdmin for the mapping between the commands and the subclasses.
 * 
 * Each Command subclass must:
 * 1) provide logic to validate the number of arguments
 * 2) for any integer/long arguments, ensure that the arguments are of type integer/long
 * 3) perform the actual command (usually on the EngineAdmin instance)
 * 
 * @author eemecoy
 *
 */
public abstract class Command {
    ...
    /**
     * simple constructor - note, no more logic should be added here - this constructor should be kept
     * as simple as possible, as it is called using reflection
     * 
     * @param args      command line arguments that the user has entered (this includes the actual name of the 
     ...
     * Checks the type of integer/long arguments
     * Assigns arguments for future use
     * 
    ...
    /**
     * The input string can be a property setting formatted like this:
     * key=value
     * Or it can be more than one property set like this:
     * key1=value1 key2=value2 (or alternatively key1=value1=key2=value2 can be used if there is a problem submitting space character in command argument)
     * Any number of pairs can be submitted. 
     * @throws InvalidArgumentsException	
     */
    String createPropertyString(final String str) throws InvalidArgumentsException {
        ...
        try {
            ...
            while (st.hasMoreTokens()){ //Add each key value pair to properties object
		...
		//change aggDate value to long
		 if(key.equals("aggDate")){
                	...
                	try {
            			...
            			throw new NumberFormatException("Please check the aggDate value: " + value+". It should be in 'yyyy-MM-dd' format");
            		}
                }
