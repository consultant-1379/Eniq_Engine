...
public class SQLSource extends TransferActionBase{
    ...
    private Vector transfTableColumns;
    ...
    private Meta_columnsFactory allColumns = null;
    ...
    *  @since  JDK1.1
    */
    public SQLSource(       Meta_versions      version,
        ...
        try {
            this.sourceTable = new Meta_source_tables(
                                            rockFact,
                                            transferActionId,
                                            (Long)null,
                                            collectionSetId,
            ...
            if (sourceTable.getTimestamp_column_id()!=null) {
                Meta_columns column = new Meta_columns(
                                                        this.getRockFact(),
                                                        sourceTable.getTimestamp_column_id(),
                                                        this.getVersionNumber(),
                                                        this.getConnectionId(),
                                                        sourceTable.getTable_id());
                this.timeStampColumnName = column.getColumn_name();
            }

            tempTableName = "META_TABLES";
            
            Meta_tables tables = new Meta_tables(
                                                this.getRockFact(),
                                                sourceTable.getTable_id(),
                                                this.getVersionNumber(),
                                                sourceTable.getConnection_id());
            ...
            this.transfTableColumns=new Vector();
            
            boolean isLookUpSet = false;
            boolean isInsideLookUpSet = false;
            ...
            for (int i=0;i<allColumns.size();i++) {
                Meta_columns col = (Meta_columns)allColumns.getElementAt(i);
                cols.put(col.getColumn_id(),col);
            }
            
            
            for (int i=0; i< this.joinedColumns.size();i++) {
                Meta_joints joint = this.joinedColumns.getElementAt(i);
                
                column = (Meta_columns)cols.get(joint.getColumn_id_source_column());
                ...
                if (joint.getPar_name()!=null) {
                    ...
                    Meta_parameter_tables whereParTable = new Meta_parameter_tables(this.getRockFact());
                    whereParTable.setVersion_number(this.getVersionNumber());
                    whereParTable.setPar_name(joint.getPar_name());
                    Meta_parameter_tables parTable = new Meta_parameter_tables(this.getRockFact(),whereParTable);
                ...
                if (joint.getTransformation_id()!=null) {
                    ...
                    Meta_transformation_rules whereRules = new Meta_transformation_rules(this.getRockFact());
                    whereRules.setVersion_number(this.getVersionNumber());
                    whereRules.setTransformation_id(joint.getTransformation_id());
                    Meta_transformation_rules rules = new Meta_transformation_rules(this.getRockFact(),whereRules);
                ...
                if(joint.getTransf_table_id()!=null) {
                    Meta_transformation_tables trTables = new Meta_transformation_tables(  this.getRockFact(),
                                                                                            joint.getTransf_table_id(),
                                                                                            this.getVersionNumber());
                                                                                            
                    if (trTables.getIs_lookup().equals("Y")) {
                        Meta_tables lookupTable = new Meta_tables(
                                                this.getRockFact(),
                                                trTables.getTable_id(),
                                                this.getVersionNumber(),
                                                trTables.getConnection_id());
                                                                        
                                                
                        Meta_columns valueCol = new Meta_columns(
                                                        this.getRockFact(),
                                                        trTables.getValue_column_id(),
                                                        this.getVersionNumber(),
                                                        trTables.getConnection_id(),
                                                        trTables.getTable_id());
                        Meta_columns keyCol = new Meta_columns(
                                                        this.getRockFact(),
                                                        trTables.getKey_column_id(),
                                                        this.getVersionNumber(),
                                                        trTables.getConnection_id(),
                                                        trTables.getTable_id());
                        if (isLookUpSet==false) {
                        ...
                        this.transfValueWhere += " "+EngineConstants.SOURCE_TABLE_ALIAS + "."+column.getColumn_name()+" = " +EngineConstants.TRANSF_OUT_TABLE_ALIAS+"."+keyCol.getColumn_name() +" ";

                        this.transfTableColumns.addElement(EngineConstants.TRANSF_OUT_TABLE_ALIAS+"."+valueCol.getColumn_name());

                    }
                    else {
                        if (trTables.getIs_lookup().equals("N")){
                            if (isInsideLookUpSet==false) {
                    
                                this.transfTablesStr += ","+DatabaseSpecific.getDaggerOwnerName(this.getRockFact())+".META_TRANSFORMATION_TABLES ";
                                this.transfTablesStr += EngineConstants.TRANSF_TABLE_ALIAS+","+DatabaseSpecific.getDaggerOwnerName(this.getRockFact());
                                this.transfTablesStr += ".META_TRANSF_TABLE_VALUES "+ EngineConstants.TRANSF_TABLE_VALUE_ALIAS;
                                if (this.transfValueWhere.length() > 0) {
                                    this.transfValueWhere += " AND ";
                                }
                                this.transfValueWhere += " "+EngineConstants.TRANSF_TABLE_ALIAS+".VERSION_NUMBER='"+this.getVersionNumber()+"' AND ";
                                this.transfValueWhere += " "+EngineConstants.TRANSF_TABLE_ALIAS+".TRANSF_TABLE_ID="+joint.getTransf_table_id()+" AND ";
                                this.transfValueWhere += " "+EngineConstants.TRANSF_TABLE_ALIAS+".VERSION_NUMBER="+EngineConstants.TRANSF_TABLE_VALUE_ALIAS+".VERSION_NUMBER AND ";
                                this.transfValueWhere += " "+EngineConstants.TRANSF_TABLE_ALIAS+".TRANSF_TABLE_ID="+EngineConstants.TRANSF_TABLE_VALUE_ALIAS+".TRANSF_TABLE_ID ";
                            }
                            this.transfValueWhere += " AND "+EngineConstants.TRANSF_TABLE_VALUE_ALIAS+".OLD_VALUE = "+EngineConstants.SOURCE_TABLE_ALIAS+ "." + column.getColumn_name();
                            this.transfTableColumns.addElement(EngineConstants.TRANSF_TABLE_VALUE_ALIAS+".NEW_VALUE");
                            
                        }
                        else {
                            this.transfTableColumns.addElement(new String(""));
                        }
                    }
                }
                else {
            ...
            if (this.batchColumnName!=null){
                ...
                for (int i=0; i< this.joinedColumns.size();i++) {
                    ...
                    if ((joint.getTarget_table_id()!=null)&&(joint.getColumn_id_target_column()!=null)) {
                        ...
                        //may be different from target connection, so the target connection ID
                        ...
                        Meta_columns col = new Meta_columns(this.getRockFact(),whereCol);
    ...
    private String buildSourceFromJoin(Meta_tables tables) {
        ...
        if (this.allColumns!=null) {
            ...
            for (int i=0;i<this.allColumns.size();i++) {
                Meta_columns column = (Meta_columns)this.allColumns.getElementAt(i);
                if (i>0) {
                    str.append(",");
                }
		String columnTableAliasName = column.getColumn_alias_name();
                str.append(columnTableAliasName);
                if (columnTableAliasName != null) {
                	String colName = columnTableAliasName.substring(
				columnTableAliasName.indexOf(".")+1,
				columnTableAliasName.length());
			String columnAliasName = column.getColumn_name();
			if ((columnAliasName != null) && (colName.equals(columnAliasName) == false)) {
				str.append(" ");
    ...
    */
    ...
    /** Returns the where clause, the last transfer condition is added to th e clause if defined. 
    *
    */
    ...
    public String getGroupByClause() {
        ...
        if (this.getJoinedColumns() != null) {
            for (int i=0; i<this.getJoinedColumns().size(); i++) {
                ...
                Meta_columns sourceColumn = (Meta_columns)this.sourceColumns.elementAt(i);
    ...
    */
    ...
    public String getSelectClause(boolean targetContainsBatchColumn){
    ...
    * @return the select clause
    */
    public String getSelectClause(boolean getPrimaryKeys, boolean getUpdatable,boolean targetContainsBatchColumn){
    ...
    * @return the select clause
     */   
    public String getSelectClause(boolean setSummary,boolean targetContainsBatchColumn){
    
        return getSelectClausePriv(false,false,setSummary,targetContainsBatchColumn);
        
    }

    public String getSourceColumn(int i,boolean getPrimaryKeys, 
                                        boolean getUpdatable,
                                        boolean setSummary) {
                                            
        String selectStr = "";
                                            
        Meta_joints metaJoint = (Meta_joints)this.joinedColumns.getElementAt(i);
        Meta_columns sourceColumn = (Meta_columns)this.sourceColumns.elementAt(i);
        if (((getPrimaryKeys==false)&&(getUpdatable==false)) || 
            ...
            if ((batchColumnName!=null) && 
                (this.batchColumnName.equals((String)this.targetColumnNames.elementAt(i)))) {
    ...
    * @return the select clause
    */
    private String getSelectClausePriv( boolean getPrimaryKeys, 
                                        boolean getUpdatable,
                                        boolean setSummary,
                                        boolean targetContainsBatchColumn) {
        ...
        if (this.joinedColumns != null) {
            ...
            for (int i=0; i<this.joinedColumns.size(); i++) {
                ...
                String columnStr = getSourceColumn(i,getPrimaryKeys,getUpdatable,setSummary);
            ...
            if ((targetContainsBatchColumn == true) &&
                ((getUpdatable)||((getUpdatable==false)&&(getPrimaryKeys==false)))) {
    ...
    *
    * @param results The database result set.
    * @param getPkValuesForWhere Are pk values added separately at the end of objVec for where part
    */
    public Vector getSelectObjVec(RockResultSet results, boolean getPkValuesForWhere) throws SQLException {
        ...
        while (results.getResultSet().next()) {
            ...
            for (int i=0; i<this.getJoinedColumns().size(); i++) {
                ...
                Object obj1 = results.getResultSet().getObject(i+1);
                int sqlType = 0;
                if (obj1 == null) {
                    ResultSetMetaData resultMetaData = results.getResultSet().getMetaData();
                ...
                if (getPkValuesForWhere) {
                    ...
                    if (joint.getIs_pk_column().equals("Y")) {
                        ...
                        Object pkObj1 = results.getResultSet().getObject(i+1);
                        if (pkObj1 == null) {
                            ResultSetMetaData resultMetaData = results.getResultSet().getMetaData();
                            sqlType = resultMetaData.getColumnType(i+1);
                        }
    ...
    */
    ...
    public void setLastTransferDate() throws EngineException {
        try {
            ...
            throw new EngineException(  EngineConstants.CANNOT_SET_TIMESTAMP,
                                        new String[]{this.sourceTableName,this.timeStampColumnName},
                                        e,
                                        this,
                                        this.getClass().getName(),
