...
public class SQLFkTable extends TransferActionBase{
    ...
    *  @since  JDK1.1
    */
    public SQLFkTable(      Meta_versions      version,
                            ...
                            Meta_fk_tables dbFkTable)throws EngineMetaDataException {
        ...
        if ((this.dbFkTable.getReplace_errors_flag()!=null) &&
            (this.dbFkTable.getReplace_errors_flag().equals("Y"))) {
            this.isReplaceable = true;
        }
        
        if ((this.dbFkTable.getFilter_errors_flag()!=null) &&
            (this.dbFkTable.getFilter_errors_flag().equals("Y"))) {
            this.isFilterable = true;
        }
        this.replaceValue = this.dbFkTable.getReplace_errors_with();
        try {
                   
            Meta_fk_table_joints whereFkJoint = new Meta_fk_table_joints(
                                            rockFact,
                                            version.getVersion_number(),
                                            connectId,
                                            this.dbFkTable.getTable_id(),
                                            (Long)null,
                                            targetTableId,
                                            (Long)null,
                                            collectionSetId,
            ...
            this.fkTableJoints = new Meta_fk_table_jointsFactory(
                                            rockFact,
                                            whereFkJoint);
                                            
            this.vecTargetColumnNames = new Vector();
            this.vecFkColumnNames = new Vector();
            for (int i=0; i < this.fkTableJoints.size(); i++) {
                Meta_fk_table_joints joint = this.fkTableJoints.getElementAt(i);
                
                tableName  = "META_COLUMNS"; 
                
                Meta_columns column = new Meta_columns(
                                                        joint.getRockFactory(),
                                                        joint.getColumn_id(),
                                                        joint.getVersion_number(),
                                                        joint.getConnection_id(),
                                                        joint.getTarget_table_id());
                                                        
                this.vecTargetColumnNames.addElement(column); 
                
                column = new Meta_columns(
                                                        joint.getRockFactory(),
                                                        joint.getColumn_id_fk_column(),
                                                        joint.getVersion_number(),
                                                        joint.getConnection_id(),
                                                        joint.getTable_id());
                this.vecFkColumnNames.addElement(column.getColumn_name()); 
            }
                                            
            tableName  = "META_TABLES"; 
            
            Meta_tables table = new Meta_tables(this.dbFkTable.getRockFactory(),
                                                        this.dbFkTable.getTable_id(),
                                                        this.dbFkTable.getVersion_number(),
                                                        this.dbFkTable.getConnection_id());
    ...
    */
    private String createFkValueGetClause(){
        ...
        if ((this.getTargetWhereClause()!=null) &&
            (this.getTargetWhereClause().length()>0)) {
               sqlStr += " WHERE ";
        }
    ...
    */
    private String getTargetCommaSeparatedColumns() {
        ...
        for (int i=0; i<this.vecTargetColumnNames.size(); i++) {
            ...
            sqlStr += (String)((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_name();
    ...
    */
    private String getFkCommaSeparatedColumns() {
        ...
        for (int i=0; i<this.vecFkColumnNames.size(); i++) {
            ...
            sqlStr += (String)this.vecFkColumnNames.elementAt(i);
    ...
    *
    */
    private String getTargetFkColWhereClause(boolean useTargetAliasName) {
        String sqlStr = "";
        ...
        for (int i=0; i<this.vecFkColumnNames.size(); i++) {
            ...
            sqlStr += EngineConstants.FK_TABLE_ALIAS + "." + (String)this.vecFkColumnNames.elementAt(i)+ " = " ;
            if (useTargetAliasName) {
                sqlStr += EngineConstants.TARGET_TABLE_ALIAS + "." + (String)((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_name();
            }
            else {
                sqlStr += this.targetTableName + "." + (String)((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_name();
            }
    ...
    */
    private String getTargetSetValuesClause() {
        ...
        for (int i=0; i<this.vecTargetColumnNames.size(); i++) {
            ...
            sqlStr += (String)((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_name();
            if ((((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_type().toUpperCase().equals(EngineConstants.NUMERIC_NAME_ORACLE)) ||
                (((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_type().toUpperCase().equals(EngineConstants.NUMERIC_NAME_SYBASE))){
    ...
    */
    private String getFkWhereClause() {
        String str = this.dbFkTable.getWhere_clause();
        if (str==null) {
            return "";
        }
        else {
    ...
    */
    public int executeFkCheck() throws EngineException {
            int numOfErrors = executeInsideDB();
            String sqlClause = "";
            try {
                ...
                if (numOfErrors > this.dbFkTable.getMax_errors().intValue()) {
                    throw new EngineException( EngineConstants.FK_ERROR_STOP_TEXT,
                                                    new String[]{""+numOfErrors+"",this.dbFkTable.getMax_errors().toString()},
                                                    null,
                                                    this,
                                                    this.getClass().getName(),
    ...
    */
    private int executeInsideDB() throws EngineException {
            try {
                ...
                RockResultSet rockResults = this.targetConnection.setSelectSQL(this.fkValueGetClause);
                ...
                while (results.next()) {
                    ...
                    for (int i = 0; i < fkTableJoints.size(); i++) {
                            Object resultObject  = results.getObject(i+2);
                            errString += ((Meta_columns)this.vecTargetColumnNames.elementAt(i)).getColumn_name()+"="+resultObject.toString();
                    }
                    errString += "#";
                ...
                throw new EngineException(  EngineConstants.CANNOT_EXECUTE,
                                            new String[]{this.fkValueGetClause},
                                            e,
                                            this,
                                            this.getClass().getName(),
                                            EngineConstants.ERR_TYPE_EXECUTION);
