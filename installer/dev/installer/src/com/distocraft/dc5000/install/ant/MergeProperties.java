...
public class MergeProperties extends Task {
  ...
  public void execute() throws BuildException {
    if(inputFile == null || outputFile == null)
      throw new BuildException("Both inputFile and outputFile must be defined");
    ...
    try {
      ...
      if(!out.canWrite() || !out.canRead() || !out.isFile())
        throw new BuildException("OutputFile cannot be read or written or not file");
      ...
      while(i.hasNext()) {
        ...
        //To check if firstDayOfTheWeek property value in static.proeprties is <> 2 don't change value
        ...
        if(key.equals("firstDayOfTheWeek")){
        	...
        	if (outprops.get(key)!=null){
        		firstDayOfTheWeek = Integer.parseInt(outprops.get(key).toString());
        	}
       	
        	if(outprops.get(key) == null || outprops.get(key).equals("2")){
                changed = true;
                outprops.put(key, inprops.get(key));
              }
        	else if (!(outprops.get(key).toString().equals(inprops.get(key).toString()))
        			&& firstDayOfTheWeek > 2 && firstDayOfTheWeek < 2){
        		changed = false;
        		continue;
        	}
        ...
        else if(outprops.get(key) == null || !(outprops.get(key).toString().equals(inprops.get(key).toString()))){
          changed = true;
          outprops.put(key, inprops.get(key));
        }
      }
