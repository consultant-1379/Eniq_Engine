...
public class DependencyCheck extends Task {
  ...
  public void execute() throws BuildException {
    ...
    if (installed_version != null) { // A version of this module is installed
    ...
    if (!"force".equals(forceflag)) {
      ...
      if (!passed)
        throw new BuildException("Installation aborted: Failed dependencies");
  ...
  private String validateRequirement(String key, Hashtable props) {
    ...
    if (operator.equals(">")) {
      ...
      if (rq_version >= 0 && inst_version < rq_version)
        return "Module " + module +" version > " + rq_version + " required.";
  ...
  private int parseVersionInfo(String src) {
    ...
    try {
      ...
      if (src.lastIndexOf("b") >= 0) {
        ...
        return Integer.parseInt(src.substring(src.lastIndexOf("b")+1));

      } else {

        System.out.println("Error parsing version info \"" + src + "\" (build number not found)");
        
      }

    } catch (Exception vne) {
      System.out.println("Error parsing version info \"" + src + "\" (" + vne.getMessage() +")");
      vne.printStackTrace();
    }
